...
def build_poisoned_training_set(is_train, args):
    transform, detransform, mean, std = build_transform(args.dataset)
    ...
    return trainset, nb_classes, mean, std


def build_testset(is_train, args):
    transform, detransform, _, _ = build_transform(args.dataset)
    print("Transform = ", transform)
...
def build_transform(dataset):
    ...
    mean = torch.tensor([0.5], )[None, :, None, None]
    std = torch.tensor([0.5], )[None, :, None, None]

    return transform, detransform, mean, std
